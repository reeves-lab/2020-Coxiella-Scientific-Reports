#Day 51 Classification iterations
#Joshua Hess
...
data_EN = read.xlsx("D51_Prism_EN_Coefficients_Log Odds.xlsx",sheet = "Prism Data")
data_EN <-data_EN[abs(data_EN$Change.in.Log.Odds.per.Unit.Increase) >= 0.05,] %>% 
  top_n(10, wt = abs(Change.in.Log.Odds.per.Unit.Increase)) %>%
  arrange(desc(abs(Change.in.Log.Odds.per.Unit.Increase))) %>%
  as.data.frame()

#Choose only challenged mice
tmp_input = data_full[,which(colnames(data_full) %in% c(data_EN$X1))]
data_tmp = cbind(tmp_input,Group)
data_input = subset(data_tmp, Group %in% c("Naive Challenge", "Vaccinated Challenge"))
