...
dir <- getwd()
doc <- read_pptx() 
matching_data = read.csv("DR3 D52 B Cells Pearsons Coefficient.csv", header = T, sep = ",", stringsAsFactors = F)
#matching_data = read.table("DR3 D51 T Cells smaller K Sorted Pearsons Data.txt", header = T, sep = "\t", stringsAsFactors = F)
rownumber = 0

for(i in 1:1200){
 #   i = 1
  if(matching_data$experiment1_count[i] < 2843 | matching_data$experiment2_count[i] < 1728){
    next()
  } else{rownumber[i] = i}
  
  
  Grp1_cluster_number = substr(matching_data$experiment1_cluster[i], 14, 18)
  Grp2_cluster_number = substr(matching_data$experiment2_cluster[i], 14, 18)
...
print(doc, target = "DR3 D52 B Cells 99000per Matching Clusters.pptx")
