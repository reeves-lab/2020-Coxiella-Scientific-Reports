...
dir <- getwd()
doc <- read_pptx() 
matching_data = read.csv("DR3 D52 T Cells Pearsons Coefficient.csv", header = T, sep = ",", stringsAsFactors = F)
#matching_data = read.table("DR3 D51 T Cells smaller K Sorted Pearsons Data.txt", header = T, sep = "\t", stringsAsFactors = F)
rownumber = 0

for(i in 1:600){
 #   i = 1
  if(matching_data$experiment1_count[i] < 1345 | matching_data$experiment2_count[i] < 689){
    next()
  } else{rownumber[i] = i}
...
print(doc, target = "DR3 D52 T Cells 99000per Matching Clusters.pptx")
