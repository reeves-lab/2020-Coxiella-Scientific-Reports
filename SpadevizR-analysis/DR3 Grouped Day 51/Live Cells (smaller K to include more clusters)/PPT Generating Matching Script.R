...
dir <- getwd()
doc <- read_pptx() 
matching_data = read.csv("DR3 D51 Live Cells Comparison Pearsons Coefficient.csv", header = T, sep = ",", stringsAsFactors = F)
rownumber = 0

for(i in 1:25000){
 #   i = 1
  if(matching_data$experiment1_count[i] < 2000 | matching_data$experiment2_count[i] < 2000 & matching_data$cor > 0.69  ){
    next()
  } else{rownumber[i] = i}
...
print(doc, target = "DR3 D51 Live Cells 99000per Matching Clusters.pptx")
