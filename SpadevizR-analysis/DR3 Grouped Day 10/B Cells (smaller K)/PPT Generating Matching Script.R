...
dir <- getwd()
doc <- read_pptx() 
matching_data = read.csv("DR3 D10 B Cells Pearsons Coefficient.csv", header = T, sep = ",", stringsAsFactors = F)
matching_data = read.table("DR3 D10 B Cells smaller K Sorted Pearsons Data.txt", header = T, sep = "\t", stringsAsFactors = F)
rownumber = 0
...
for(i in 1:776){
 #   i = 1
  ...
  Grp1_scatter_file = paste(dir, "/Group1_Scatterplots_new/Cluster_", Grp1_cluster_number, ".jpeg", sep = "")
  Grp2_scatter_file = paste(dir, "/Group2_Scatterplots_new/Cluster_", Grp2_cluster_number, ".jpeg", sep = "")
...
print(doc, target = "DR3 D10 B Cells 99000per Matching Clusters SORTED all slides.pptx")
