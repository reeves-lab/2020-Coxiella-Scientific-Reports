...
matching_file_data = read.table("Matching Clusters for Visualization (SK).txt", header = T, sep = "\t", stringsAsFactors = F)
...
for(i in 1:18){
 # i=1
  ...
  Grp2_cluster_number = substr(matching_file_data$Experiment2_Cluster[i], 1, 4)
  ...
  Volcano_Plot_CvV = paste(dir, "/SpadevizR Analysis and Volcano Plots/Control vs Vaccinated", ".tiff", sep = "")
  # Volcano_Plot_CvVU = paste(dir, "/Volcano_Plot/Control vs Vax_Unchal", ".tiff", sep = "")
  # Volcano_Plot_NCvVC = paste(dir, "/Volcano_Plot/Naive_Chal vs Vax_Chal", ".tiff", sep = "")
  # Volcano_Plot_VUvVC = paste(dir, "/Volcano_Plot/Vax_Unchal vs Vax_Chal", ".tiff", sep = "")
  ...
  Volcano_Plot_magick_CvV = image_read(Volcano_Plot_CvV)
  ...
  image_write(Volcano_Plot_magick_CvV, path = "Control vs Vaccinated.tiff", format = "tiff")
  ...
  Volcano_Plot_CvV = paste(dir, "/Control vs Vaccinated.tiff", sep = "")
  ...
  #VOLCANO PLOTS SIZE AND LOCATION
    ph_with_img_at(src = Volcano_Plot_CvV, height = 2.1, width = 3.89, left = 6.07, top = 0.0)
  # ph_with_img_at(src = Volcano_Plot_CvNC, height = 2.1, width = 1.22, left = 6.07, top = 0.0)%>%
  # ph_with_img_at(src = Volcano_Plot_CvVU, height = 2.1, width = 1.22, left = 7.03, top = 0.0)%>%
  # ph_with_img_at(src = Volcano_Plot_NCvVC, height = 2.1, width = 1.22, left = 7.99, top = 0.0)%>%
  # ph_with_img_at(src = Volcano_Plot_VUvVC, height = 2.1, width = 1.22, left = 8.95, top = 0.0)
  
  
  file.remove(Volcano_Plot_CvV, Grp1_cluster_file, Grp2_cluster_file, Grp1_scatter_file, Grp2_scatter_file, Grouped_cluster_file, Grouped_scatter_file)
  print(i)
}

#GENERATES POWERPOINT 
print(doc, target = "DR3 D35 T Cell 99000per genbyR.pptx")
