...
dir <- getwd()
doc <- read_pptx() 
matching_data = read.csv("DR3 D35 T Cells Pearsons Coefficient.csv", header = T, sep = ",", stringsAsFactors = F)
matching_data = read.table("DR3 D35 T Cells smaller K Sorted Pearsons Data.txt", header = T, sep = "\t", stringsAsFactors = F)
rownumber = 0

for(i in 1:1500){
 #   i = 1
  if(matching_data$experiment1_count[i] < 1844 | matching_data$experiment2_count[i] < 2346 & matching_data$cor >= 0.7){
    next()
  } else{rownumber[i] = i}
...
print(doc, target = "DR3 D35 T Cells 99000per Matching Clusters corrected cutoff.pptx")
