...
run.ML.iterations = function(df,name_of_response,export_results,num_cores,max_size){
  ...
  comb_list = lapply(seq_len(max_size), FUN = function(x)combn(colnames(tmp_data), x)) 
  ...
  for (j in 1:length(result_list)){
    for (k in 1:length(result_list[[j]])){
      for (h in 1:length(result_list[[j]][[k]])){
        ...
        tmp_confusion_frame$Coefficient = paste(as.character(result_list[[j]][[k]][[h]]$finalModel$scaling),collapse = " ")
  ...
  if (export_results ==TRUE){
    write.xlsx(LDA_export_table,paste("LDA_results_comb.xlsx"))
  }
  
  #Return the result_list so that you can extract any model you wish
  return(list(result_list,LDA_export_table))
}
