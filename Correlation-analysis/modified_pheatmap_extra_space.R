...
find_coordinates = function(n, gaps, m = 1:n){
  ...
  size = (1 / n) * (unit(1, "npc") - length(gaps) * unit("14", "bigpts"))
  
  gaps2 = apply(sapply(gaps, function(gap, x){x > gap}, m), 1, sum) 
  coord = m * size + (gaps2 * unit("14", "bigpts"))
...
draw_rownames = function(rown, gaps, ...){
  ...
  res = textGrob(rown, x = unit(3, "bigpts"), y = y, vjust = 0.5, hjust = 0, gp = gpar(col = "black"))
...
find_gaps = function(tree, cutree_n){
  v = cutree(tree, h=cutree_n)[tree$order]
...
#' 
#' @export
pheatmap_modified = function(mat, 
  ...
  if((class(cluster_rows) == "hclust") || cluster_rows){
    ...
    if(!is.na(cutree_rows)){
      ...
      mat1$group = 0
      for(i in 1:(length(gaps_row)+1)){
        if(i == 1){
          mat1$group[c(1:gaps_row[i])] = paste("group_", i, sep = "")
        }
        if(i >1 & i <= length(gaps_row)){
          mat1$group[(gaps_row[i-1] + 1) : gaps_row[i]] = paste("group_", i, sep = "")
        }
        if(i > length(gaps_row)){
          mat1$group[(gaps_row[i-1] + 1) : nrow(mat1)] = paste("group_", i, sep = "")
        }
      }
      mat1 = mat1[, c(ncol(mat1), 1:(ncol(mat1)-1))]
      write.table(mat1, "group_data_testJH.txt", sep = "\t", row.names = T, quote = F)
      #print(tree_row$height)
      print(paste("Min of treeheight: ", min(tree_row$height), sep = ""))
      print(paste("Max of treeheight: ", max(tree_row$height), sep = ""))
    }
    else{
      gaps_row = NULL
    }
  ...
  gt = heatmap_motor(mat, 
                     ...
                     main = main, fontsize = fontsize, fontsize_row = fontsize_row, fontsize_col = fontsize_col, fmat = fmat, fontsize_number = fontsize_number, number_color = number_color, gaps_row = gaps_row, gaps_col = gaps_col, labels_row = labels_row, labels_col = labels_col, ...)
  dev.off()
